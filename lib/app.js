...
const ItemContainer = require('./views/item/container');
...
class App extends React.Component {
  ...
  componentDidMount() {
    debug('mounted');
    Linking.getInitialURL().then(this.onDeepLink.bind(this));
    this.startScanning();
  }
  ...
  renderScene(route, navigator) {
    debug('render scene', route.type, this.props.openItem);
    ...
    switch (route.type) {
      ...
      case 'item':
      return <ItemContainer navigator={navigator} />;
  ...
  onDeepLink(url) {
    ...
    switch (parsed.host) {
      case 'item':
        this.props.setOpenItem({ url: parsed.query.url });
