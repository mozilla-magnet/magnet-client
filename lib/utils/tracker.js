...
const tracker = require('./native-tracker');
...
class Tracker {
  ...
  itemLost(url) {
    this.event('system', 'item-lost', {
      label: `originalUrl:${url}`,
    });
  }

  itemFetched(originalUrl, finalUrl) {
    this.event('system', 'item-fetched', {
  ...
  channelSubscribed(channelId) {
    debug('channel subscribed', channelId);
    this.event('interaction', 'channel-subscribed', {
      label: `channelId:${channelId}`,
    });
  }

  channelUnsubscribed(channelId) {
    debug('channel unsubscribed', channelId);
    this.event('interaction', 'channel-unsubscribed', {
      label: `channelId:${channelId}`,
    });
  }
}
