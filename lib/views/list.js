...
class ListView extends React.Component {
  ...
  renderMessage() {
    debug('render message', this.props.scanning);
  ...
  getItemViewportOffsetY(item) {
    ...
    return measureLayout(item, this.refs.content)
      .then(({ y }) => y - this.scrollY);
  }
}
 ...
 */
 ...
 /**
  * The react-native docs tell you to use
  * the 'NativeMethodsMixin' module to
  * measure nodes. But we got a warning
  * that the module was using deprecated
  * `isMounted()` API. So now we're using
  * the `UIManager` module directly.
  *
  * @param  {Object} node
  * @param  {Object} relativeTo
  * @return {Promise
  */
 function measureLayout(node, relativeTo) {
