...
var MagnetWebView = require('../../magnet-web-view');
...
/**
 * The minimum allowed height. Also used
 * as a default if we're unable to derive
 * one from oembed data.
 *
 * @type {Number}
 */
...
class ContentViewEmbed extends React.Component {
  ...
  render() {
    ...
    var scrollEnabled = !!expanded;
    var source = this.getSource();
    var pointerEvents = expanded ? 'auto' : 'none';
    var flex = expanded && this.state.isOverflowing ? 1 : 0;

    debug('render', source, pointerEvents);

    return (
      <View
        style={[styles.root, { flex: flex }]}
  ...
  getHeight(width) {
    ...
    return Math.round(width * aspect) || MIN_HEIGHT;
  }

  /**
   * We want to avoid re-layout if possible
   * as it can disrupt the users scroll
   * position. So to do this we only
   * resize the collapsed embed tile
   * if the content is between the
   * max/min height constants.
   *
   * @param {Number} contentHeight
   */
  setHeight(contentHeight) {
    debug('set height', contentHeight);
    var height = this.state.height;
    var isOverflowing = false;

    if (contentHeight > MAX_HEIGHT) {
      isOverflowing = true;
    } else if (contentHeight > MIN_HEIGHT) {
    ...
    debug('height set', height, isOverflowing);
  }
